...
const ProductSlider = ({ title, tagline }) => {
    ...
    const { loading, products, suggestedProducts } = useSelector((state) => state.products);
    const sliderProducts = title === "Suggested for You" ? suggestedProducts : products;
    return (
        <section className="bg-white w-full shadow overflow-hidden">
            ...
            {loading ? null :
                <Slider {...settings} className="flex items-center justify-between p-1 " aria-hidden="true">
                    {sliderProducts && getRandomProducts(sliderProducts, 12).map((product) => (
